...
import { ArrowLeft, Send, Sparkles, Terminal, RefreshCcw, FileText, X } from 'lucide-react';
  ...
  return (
    <div className="workspace-page">
      <header className="workspace-header">
        ...
        <button className="mobile-transcript-toggle" onClick={() => setTranscriptOpen(true)}>
          <FileText size={18} />
          {transcripts.length > 0 && <span className="transcript-count">{transcripts.length}</span>}
        </button>
      </header>
          ...
          <div className="transcript-content">
            {transcripts.length === 0 ? (
              ...
              transcripts.map((t, idx) => (
                <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: idx * 0.05 }} key={idx} className="transcript-item">
        ...
        <div className="chat-panel">
          <div className="chat-content">
            {chatHistory.length === 0 && (
              <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="chat-empty">
                <div className="chat-empty-icon"><Sparkles size={24} color="var(--text-primary)" strokeWidth={1.5} /></div>
                <h3 className="chat-empty-title">Intelligence Module</h3>
                <p className="chat-empty-desc">Query the meeting vector database. The system synthesizes answers directly from transcribed semantics.</p>
            ...
            <AnimatePresence initial={false}>
              {chatHistory.map((chat, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="chat-message-container">
                  <div className="msg-row-user">
                    <div className="msg-bubble-user"><p className="msg-text-user">{chat.question}</p></div>
                  </div>
                  <div className="msg-row-ai">
                    <div className="msg-bubble-ai">
                      <div className="msg-ai-highlight"></div>
                      <div className="msg-ai-icon-container"><div className="msg-ai-icon"><Sparkles size={12} color="var(--text-primary)" /></div></div>
          ...
          <div className="chat-input-area">
            ...
            <form onSubmit={handleAsk} className="chat-form">
              <input type="text" value={question} onChange={(e) => setQuestion(e.target.value)} placeholder="Query workspace..." disabled={isAsking} className="chat-input" />
              <button type="submit" disabled={isAsking || !question.trim()} className="chat-send-btn">
                <Send size={16} />
              </button>
            </form>
          </div>
      ...
      <AnimatePresence>
        {transcriptOpen && (
          <motion.div className="ws-transcript-overlay" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setTranscriptOpen(false)}>
            <motion.div className="ws-transcript-popup" initial={{ y: '100%' }} animate={{ y: 0 }} exit={{ y: '100%' }} transition={{ type: 'spring', damping: 30, stiffness: 280 }} onClick={e => e.stopPropagation()}>
              <div className="ws-popup-header">
                <div className="ws-panel-header">
                  <div className="ws-panel-title">
                    <Terminal size={14} /> Protocol Transcript
                  </div>
                  <button onClick={fetchData} className="ws-icon-btn" title="Refresh">
                    <RefreshCcw size={14} />
                  </button>
                </div>
                <button onClick={() => setTranscriptOpen(false)} className="ws-icon-btn"><X size={16} /></button>
              </div>
              <div className="ws-transcript-scroll">
                {transcripts.length === 0 ? (
                  <div className="ws-empty-state">
                    <Terminal size={28} strokeWidth={1} />
                    <p>AWAITING_DATA_STREAM...</p>
                    <span>Transcripts appear after a meeting is processed.</span>
                  </div>
                ) : (
                  transcripts.map((t, idx) => (
                    <div key={idx} className="ws-transcript-item">
                      <span className="ws-transcript-time">
                        {new Date(t.createdAt).toLocaleTimeString([], { hour12: false, hour: '2-digit', minute: '2-digit' })}
                      </span>
                      <div className="ws-transcript-body">
                        <span className="ws-transcript-speaker">{t.userId?.username || 'SYS'}</span>
                        <span className="ws-transcript-text">{t.text}</span>
                      </div>
                    </div>
                  ))
                )}
